...
function ShoppingList() {
  ...
  const [activeGroup, setActiveGroup] = useState(''); // Filtro de grupo
  ...
  // Filtrar los productos por grupo activo ANTES de exportar o mostrar
  ...
  return (
    <div style={{ ...styles.container, backgroundColor: theme.background }}>
      ...
      <div style={styles.content}>
        ...
        {/* SELECTOR DE GRUPO EN LISTA DE COMPRAS */}
        {groups.length > 0 && (
          <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
            <span style={{ color: theme.textMuted, fontSize: '14px', fontWeight: '500' }}>Viendo lista de:</span>
            <select
              value={activeGroup}
        ...
        {displayedProducts.length === 0 ? (
          <div style={{ ...styles.emptyState, backgroundColor: theme.cardBg, border: `1px solid ${theme.border}` }}>
            ...
            <h2 style={{ ...styles.emptyTitle, color: theme.text }}>La lista de este grupo está vacía</h2>
          ...
          <>
            <div style={styles.headerInfo}>
              ...
              <div style={styles.badge}>{displayedProducts.length} {displayedProducts.length === 1 ? 'producto' : 'productos'}</div>
            </div>
            ...
            <div style={styles.exportSection}>
              ...
              <p style={{ ...styles.exportHint, color: theme.textMuted }}>Copia la lista para enviarla por WhatsApp o guardarla en notas</p>
            </div>
          </>
        )}
      </div>
...
const styles = {
  ...
  badge: { backgroundColor: '#8C7AE6', color: 'white', padding: '8px 20px', borderRadius: '20px', fontSize: '14px', fontWeight: '600', boxShadow: '0 2px 8px rgba(140, 122, 230, 0.3)' },
  productList: { display: 'flex', flexDirection: 'column', gap: '15px' },
  productCard: { borderRadius: '16px', padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '20px', boxShadow: '0 1px 3px rgba(0, 0, 0, 0.03)', transition: 'all 0.3s ease' },
  productInfo: { flex: 1 },
  productHeader: { display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '15px' },
  productName: { margin: 0, fontSize: '20px', fontWeight: '600' },
  categoryBadge: { backgroundColor: '#F7E7FA', color: '#8C7AE6', padding: '4px 12px', borderRadius: '12px', fontSize: '12px', fontWeight: '600', textTransform: 'capitalize' },
  stockInfo: { display: 'flex', gap: '30px', marginBottom: '10px' },
  stockItem: { display: 'flex', alignItems: 'center', gap: '8px' },
  stockLabel: { fontSize: '14px', fontWeight: '500' },
  stockValue: { fontSize: '18px', fontWeight: '700' },
  notes: { fontSize: '13px', marginTop: '8px' },
  notesLabel: { fontWeight: '600' },
  productActions: { display: 'flex', alignItems: 'center' },
  removeBtn: { padding: '12px 24px', background: 'linear-gradient(135deg, #8C7AE6 0%, #6B5BC9 100%)', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '15px', fontWeight: '600', transition: 'all 0.3s ease', boxShadow: '0 4px 12px rgba(140, 122, 230, 0.3)', whiteSpace: 'nowrap' },
  ...
  emptyState: { textAlign: 'center', padding: '80px 20px', borderRadius: '20px', boxShadow: '0 1px 3px rgba(0, 0, 0, 0.03)' },
  emptyIcon: { fontSize: '80px', marginBottom: '20px' },
  emptyTitle: { fontSize: '28px', fontWeight: '700', marginBottom: '10px' },
